...
def plot_animation(i, data, label, prediction, nom):
    ...
    #the list of points that should appear in different color each frame (list of 100 lists of len 10) 
    points_color_indices = [[231, 220, 230,  24, 232, 221,  23, 219, 222,  25],
    ...
    def update(frame):    
        ...
        colors = ['yellow' if idx in joints_color[frame] else 'blue' for idx in range(len(x_data[frame]))]
    ...
    animation.save(f'self_supervised_learning/dev/ProjetCassiopee/src/visualisation/mocaplab_points_color/{nom}.gif', writer='pillow')
    plt.close()
